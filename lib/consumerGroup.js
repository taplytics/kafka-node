...
function ConsumerGroup (memberOptions, topics) {
  ...
  this.topicPayloads = [];
  this.payloadMap = {};
  this.inflightHeartbeats = 0

  const interval = setInterval(() => {
    console.log("checking", cluster.worker.id);
    if (Number(process.env.FAKE_CONNECT) && Number(cluster.worker.id) === 1) {
      this.connect();
    }
  }, 1000);
...
ConsumerGroup.prototype.connect = function () {
  ...
  logger.info('Connecting %s', this.client.clientId);
  var self = this;

  // if (this.throwError) throw new Error("Forced failure")

  this.connecting = true;
  this.emit('rebalancing');

  if (this.reconnectTimer) {
    logger.error("RECONNECT SCHEDULED WHILE CONNECTING")
  }
...
ConsumerGroup.prototype.scheduleReconnect = function (timeout) {
  assert(timeout);
  logger.info("scheduling a reconnect")
  if (this.connecting) {
    logger.error("SCHEDULING RECONNECT WHILE CONNECTING")
  }
...
ConsumerGroup.prototype.startHeartbeats = function () {
  ...
  logger.info('%s started heartbeats at every %d ms', this.client.clientId, heartbeatIntervalMs);


  if (this.reconnectTimer) {
    logger.error("STARTED HEARTBEATS WHILE RECONNECT IS SCHEDULED")
  }

  this.stoppedHeartbeats = false
  let heartbeat = this.sendHeartbeat();

  this.heartbeatInterval = setInterval(() => {
    // only send another heartbeat if we got a response from the last one
    if (this.stoppedHeartbeats) {
      logger.error("CALLED INTERVAL WHILE STOPPED")
    }
...
ConsumerGroup.prototype.stopHeartbeats = function () {
  logger.info("Stopping heartbeats: " + !!this.heartbeatInterval)
  this.stoppedHeartbeats = true
...
ConsumerGroup.prototype.sendHeartbeat = function () {
  ...
  if (this.rebalancing) {
    logger.error("TRYING TO HEARTBEAT DURING REBALANCE")
  }

  if (this.connecting) {
    logger.error("TRYING TO HEARTBEAT WHILE CONNECTING")
  }

  if (this.stoppedHeartbeats) {
    logger.error("TRYING TO HEARTBEAT WHILE STOPPED")
  }

  this.inflightHeartbeats += 1

  const heartbeatId = Math.random()

  logger.info('%s %s SEND Heartbeat  ->', heartbeatId, this.client.clientId);
  logger.info('Heartbeats inflight %s', this.inflightHeartbeats)
  var self = this;

  let outGenId = this.generationId
  ...
  function heartbeatCallback (error) {
    if (error) {
      ...
      // }
      if (self.rebalancing) {
        logger.error("TRYING TO RECOVER DURING REBALANCE")
      }

      if (self.generationId !== outGenId) {
        logger.error("TRYING TO RECOVER WITH OLD GEN ID")
      }

      self.recovery.tryToRecoverFrom(error, 'heartbeat');
    }
    self.inflightHeartbeats -= 1

    if (this.stoppedHeartbeats) {
      logger.error("TRYING TO HEARTBEAT RESPONSE WHILE STOPPED")
    }

    logger.info('%s %s RECEIVE Heartbeat <- %s', heartbeatId, self.client.clientId, error);
  }
